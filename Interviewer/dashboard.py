# Dashboard for the AI interviewer with RAG
...
def create_session(settings, session_id=None):
    ...
    if response.status_code == 200:
        data = response.json()
        # Update resume path if available
        if "resume_path" in data:
            st.session_state.resume_path = data["resume_path"]
...
def get_resume_info(session_id):
    """Get information about the resume for this session"""
    response = requests.get(f"{API_URL}/resume-info/{session_id}")
    if response.status_code == 200:
        return response.json()
    else:
        st.error(f"Error getting resume info: {response.text}")
        return None
...
def render_setup_ui():
    ...
    with st.form("interview_settings"):
        ...
        resume_url = st.text_input("Resume URL (S3 or HTTP)", value="", 
                                  help="Enter an S3 URL (s3://bucket/path.pdf) or an HTTP URL to the candidate's resume")
        ...
        if submitted:
            ...
            settings = {
                ...
                "resume_url": resume_url if resume_url else None  # Add resume URL to the settings
            }
...
def render_interview_ui():
    ...
    if st.session_state.resume_path:
        st.success(f"Resume loaded: {st.session_state.resume_path}")
...
def render_admin_tools():
    ...
    if st.session_state.session_id:
        ...
        # Resume info
        if st.session_state.resume_path:
            st.sidebar.info(f"Resume: {st.session_state.resume_path}")
            
            # Add resume query tool for admins
            st.sidebar.markdown("### Resume Query Tool")
            resume_query = st.sidebar.text_input("Query resume content")
            if st.sidebar.button("Query Resume"):
                if resume_query:
                    response = send_message(
                        st.session_state.session_id, 
                        f"query_resume:{resume_query}", 
                        message_type="system"
                    )
                    if response:
                        st.sidebar.success("Query sent, check response below:")
        ...
        # Get resume info
        if st.sidebar.button("Get Resume Info"):
            resume_info = get_resume_info(st.session_state.session_id)
            if resume_info:
                if resume_info.get("resume_available", False):
                    st.sidebar.success("Resume is available")
                    st.sidebar.json(resume_info)
                else:
                    st.sidebar.warning("No resume available for this session")
                    if "error" in resume_info:
                        st.sidebar.error(resume_info["error"])
        ...
        if st.sidebar.button("Get Conversation History"):
            ...
            if history:
                ...
                # Display resume path if available
                if "resume_path" in history and history["resume_path"]:
                    st.sidebar.info(f"Resume path: {history['resume_path']}")
...
def main():
    ...
    with st.expander("About this application"):
        ...
        **Features:**
        ...
        - Upload a candidate's resume to personalize questions based on their experience
        ...
        **Usage:**
        ...
        3. Provide a resume URL (S3 or HTTP) to enable resume-based questioning
        4. Start the interview and answer questions as a candidate would
        5. Admin tools are available in the sidebar for testing and monitoring
        """)
