...
def render_interview_ui():
    ...
    with col2:
        ...
        if st.session_state.interview_active:
            status_text += f" - Questions Asked: {st.session_state.question_count}"
...
def render_admin_tools():
    ...
    if st.session_state.session_id:
        ...
        # Security info section (simplified to handle missing get_cheat_attempts())
        security_info = get_security_info(st.session_state.session_id)
        if security_info:
            st.sidebar.markdown("### Security Information")
            st.sidebar.info(f"Session active: {security_info['active']}")
